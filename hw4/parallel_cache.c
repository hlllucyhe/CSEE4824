...
static void merge_tiled(int* arr, int left, int mid, int right, int* temp_full) {
    int n1 = mid - left + 1;
    int n2 = right - mid;
    ...
    int k = left;  // write position into arr
...
static void mergeSort_serial_cache(int* arr, int left, int right, int* temp) {
...
// ---------- Parallel + cache-tiled merge sort ----------
static void mergeSort_parallel_cache(int* arr, int left, int right, int* temp) {
    if (left >= right) return;
    ...
    // Parallel tasks for recursive calls
...
#pragma omp parallel
    {
#pragma omp single
        mergeSort_parallel_cache(arr, 0, (int)n - 1, temp);
    }
